 ...
 * implied warranty.
 *
 * Modified by Antonio Costa to work for the SCN converter
 *****************************************************************************/
...
#define ELEMENT_TABLE "/net/asterix/users/acc/new/pdb2scn/element.tab"
#endif
...
FILE *pdbfile, *output;
{
	...
	if (name[0]==' ')
		fprintf (output,"ATOM_%c(%g,%g,%g)\n",name[1],x,y,z);
	else
		fprintf(output,"ATOM_%c%c(%g,%g,%g)\n",name[0],name[1],x,y,z);
}
...
FILE *output;
{
	...
	if (name[0]==' ')
		fprintf(output,"#define ATOM_%c(x,y,z) \\\n",name[1]);
	else
		fprintf(output,"#define ATOM_%c%c(x,y,z) \\\n ",
		        name[0],name[1]);
	fprintf(output,"sphere\\\n");
	fprintf(output," surface %g %g %g %s\\\n",r,g,b,thisatom->shade);
	fprintf(output," data x y z %g\n\n",d);
}
...
#define tan_fov_angle 0.46630766 /* tangent of roughly half the fovea angle */
	...
	cam_x= look_z;
	cam_y= look_y;
	cam_z= zmax + (0.5/tan_fov_angle)*
		( (xmax-xmin > ymax-ymin) ? (xmax-xmin) : (ymax-ymin) );

	hither= 0.25*( zmax - cam_z );
	yon= 2.0*( zmin - cam_z );

	light_x= cam_x;
	light_y= cam_y;
	light_z= (cam_z+look_z)/2.0;
...
%% hither %g\n\
%% yon %g\n\
light point %g %g %g white\n\
",
	cam_x, cam_y, cam_z, look_x, look_y, look_z, hither, yon,
