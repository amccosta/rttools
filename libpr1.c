...
lib_output_viewpoint(from, at, up,
    ...
    int    resx, resy;
{
    COORD3 axis, myup1, myup;
    COORD4 viewvec, rightvec;
    MATRIX m1;
    double twist, xang, yang, len;
	...
	case OUTPUT_RIB:
	    ...
	    /* RMAN is left handed !*/
	    /* Transform axis to LH */
	    SUB3_COORD3(axis, at, from);
	    lib_normalize_vector(axis);
	    axis_to_z(axis, &xang, &yang);

	    /* Calculate transformed up vector */
	    lib_create_rotate_matrix(m1, Y_AXIS, -yang/180*PI);
	    lib_transform_vector(myup, up, m1);
	    lib_create_rotate_matrix(m1, X_AXIS, -xang/180*PI);
	    lib_transform_vector(myup1, myup, m1);
	    lib_create_rotate_matrix(m1, Y_AXIS, PI);
	    lib_transform_vector(myup, myup1, m1);
	    twist= 90 - (180 * atan2(myup[Y], myup[X]) / PI);
	    ...
	    tab_indent();
	    fprintf(gOutfile, "Rotate 180 0 1 0\n");
	    tab_indent();
	    ...
	    fprintf(gOutfile, "Color 200 200 200\n");
