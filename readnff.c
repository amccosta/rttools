...
FILE *fp;
{
	...
	return;
fmterr:
	fprintf(stderr, "NFF view syntax error\n");
	exit(1);
}
...
FILE *fp;
{
   ...
   if (fscanf(fp, "%f %f %f",&x, &y, &z) != 3) {
	fprintf(stderr, "light source syntax error\n");
	exit(1);
	}
...
FILE *fp;
{
   ...
   if (fscanf(fp, "%f %f %f",&r, &g, &b) != 3) {
	fprintf(stderr, "background color syntax error\n");
	exit(1);
	}
...
FILE *fp;
{
   ...
   if (fscanf(fp, "%f %f %f",&r, &g, &b) != 3) {
	fprintf(stderr, "fill color syntax error\n");
	exit(1);
	}
	SET_COORD3(acolor,r,g,b);

	if (fscanf(fp, "%f %f %f %f %f", &kd, &ks, &phong_pow, &t, &ior) != 5) {
		fprintf(stderr, "fill material syntax error\n");
		exit(1);
	}

	/* ooh yuck, some parms not input, so I hard-coded them for now */
	ka = 0.1;
	shine = 0.5;
...
FILE *fp;
{
	...
	if (fscanf(fp, " %f %f %f %f %f %f %f %f", &x0, &y0, &z0, &r0,
			&x1, &y1, &z1, &r1) != 8) {
		fprintf(stderr, "cylinder or cone syntax error\n");
		exit(1);
	}
	if ( r0 < 0.0) {
		r0 = -r0;
...
FILE *fp;
{
	...
	if (fscanf(fp, "%f %f %f %f", &x, &y, &z, &r) != 4) {
		fprintf(stderr, "sphere syntax error\n");
		exit(1);
	}
